    ...
    try {
        ...
        window.fetch = async (input: RequestInfo, init?: RequestInit) => {
            try {
                ...
                const isInertiaReq = headers.has('X-Inertia') || headers.has('x-inertia');
                const res = await _origFetch(input, init);
                if (isInertiaReq) {
                    const hasInertiaResp = !!(res.headers.get('X-Inertia') || res.headers.get('x-inertia'));
                    const ct = (res.headers.get('content-type') || '').toLowerCase();
                    if (!hasInertiaResp && ct.includes('application/json')) {
                        console.groupCollapsed('Dev: Inertia received JSON for', String(input));
                        console.log('Request init:', init);
                        console.log('Response status:', res.status);
                        console.log('Response headers:', Object.fromEntries(res.headers.entries()));
                        try { res.clone().text().then(b => console.log('Response body:', b)); } catch (e) { /* ignore */ }
                        console.trace();
                        console.groupEnd();
                    }
                }
        ...
        proto.open = function(this: any, method: string, url: string, async?: boolean, user?: string, password?: string) {
            ...
            try {
                this.addEventListener('load', function(this: any) {
                    try {
                        ...
                        const isInertiaReq = hdrs['X-Inertia'] || hdrs['x-inertia'];
                        const respInertia = this.getResponseHeader && (this.getResponseHeader('X-Inertia') || this.getResponseHeader('x-inertia'));
                        const ct = (this.getResponseHeader && this.getResponseHeader('content-type')) || '';
                        if (isInertiaReq && !respInertia && ct.toLowerCase().includes('application/json')) {
                            console.groupCollapsed('Dev: Inertia XHR received JSON for', this._otadev_url);
